...
import { enrichCourseWithYouTubeRetries, type GeneratedCourse } from "@/lib/ai";
...
export async function POST(request: Request) {
    try {
        ...
        const maxAttempts = Number.isFinite(requestedAttempts)
            ? Math.min(3, Math.max(1, Math.floor(requestedAttempts)))
            : 2;

        const course = await enrichCourseWithYouTubeRetries(rawCourse, topic, {
            maxAttempts,
            retryDelayMs: 900,
        });
        ...
        const message = error instanceof Error ? error.message : "Failed to retry YouTube enrichment";
        console.error("YouTube retry error:", error);
        return NextResponse.json(
            { error: message },
            { status: 500 }
        );
